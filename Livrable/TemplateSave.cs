...
namespace Livrable
{
    ...
    public void SaveDiff(string sourceDirName, string destDirName, bool copySubDirs)
    {
        ...
        foreach (FileInfo file in files)
        {
            Console.WriteLine(Save.horodatage);
            comp = DateTime.Compare(file.LastWriteTime, Save.horodatage);
            if (comp > 0)
            {
                string tempPath = Path.Combine(destDirName, file.Name);
                file.CopyTo(tempPath, false);
