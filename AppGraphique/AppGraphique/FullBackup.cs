...
namespace Projet.Model
{
    class FullBackup : IBackup
    {
        ...
        public void Sauvegarde(string TextboxSourceFR, string TextboxDestinationFR, bool copyDirs, int getStateIndex, long fileCount, int getIndex, string getName)
        {
            //Initialize a timer which determine the backup time
            Stopwatch stopWatch = new Stopwatch();
            stopWatch.Start();
            // Get the subdirectories for the specified directory.
            DirectoryInfo dir = new DirectoryInfo(TextboxSourceFR);

            if (!dir.Exists)
            {
                throw new DirectoryNotFoundException("Source directory does not exist or could not be found: " + TextboxSourceFR);
            }
            //condition in order to create the source subdirectories in the destination directory
            if (copyDirs)
            {
                CreateDirs(TextboxDestinationFR, dir.GetDirectories());
            }
